...
OriGene is the strongest single peer-reviewed-ish precedent for the proposal's agent architecture — a multi-role feedback loop, wet-lab validated, in biology. **A caveat to the analogy:** OriGene operates on target-identification (a biology problem with verifiable text-and-database evidence); MB Finder v2 operates on structure-design (a chemistry problem with verifiable synthesis-and-assay evidence). The architectural primitives transfer; **the empirical scaling laws (e.g. OriGene's 62.81 % → 78.39 % LitQA accuracy at 9× iteration budget shown in the table above) do NOT transfer to chemistry-agent contexts — those numbers were measured on text-and-database retrieval over LitQA, not synthesis planning or candidate ranking**, which have different cost surfaces, different verifier sharpness, and different failure modes. The proposal therefore inherits OriGene's role decomposition and feedback-loop pattern, but specialises the verification rubric (chemoinformatic sanitization, drug-likeness, novelty, retrosynthesis, geometry) to the chemistry context — see the five chemistry verifiers in ADR-009 — and treats the iteration-budget / accuracy curve as something to *measure*, not to *inherit*.
...
- **MLX-LM on lab Macs** for offline / per-chemist literature triage on confidential structures (**Llama-3.2-3B-Instruct 4-bit at 152 tok/s** on M-series Mac via MLX-LM — specific to that model + quantisation, not a generic 3 B-class figure; Simon Willison, Feb 2025).
...
The benchmark Anthropic reports is striking: *"This reduces the token usage from 150,000 tokens to 2,000 tokens — a time and cost saving of 98.7 %."* That figure is from Anthropic's specific worked example (a Salesforce + Google Drive workflow with thousands of available tools); the magnitude on metallodrug-discovery workloads will depend on the agent's tool count and data shape, but the qualitative win (one code block instead of N tool calls) reproduces across domains. Cloudflare independently confirmed the same pattern, calling it "Code Mode."

Specifically for fragment-assembly screening, this is the canonical pattern adopted by the proposal: the 427 de novo scaffolds × 4 cell lines × 3 protein targets = 5,124 inference calls is **formalised as a single MCP code-execution block** authored by the Design agent. The Design agent emits one Python snippet that (a) iterates over the 5,124 (scaffold, cell-line, target) tuples, (b) calls inference-api per tuple, (c) ranks the results, (d) returns only the top-N ranked list to the Supervisor. Sandbox execution (per ADR-013) provides the isolation boundary. **This is the architectural pattern for any fan-out screen the proposal adopts**: never N-tool-calls-per-message; always one code block returning a ranked list. Without code execution, the same workflow would burn the orchestrator's token budget on intermediate predictions.
